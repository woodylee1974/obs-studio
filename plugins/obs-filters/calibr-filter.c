...
static const char *calibr_filter_get_name(void *unused)
{
	UNUSED_PARAMETER(unused);
	return obs_module_text("CalibFilter");
}

static void calibr_filter_image_unload(struct calibr_filter_data *filter)
{
	obs_enter_graphics();
...
static void calibr_filter_image_load(struct calibr_filter_data *filter)
{
	calibr_filter_image_unload(filter);
...
static void calibr_filter_update(void *data, obs_data_t *settings)
{
	struct calibr_filter_data *filter = data;
	...
	filter->image_file = obs_module_file("mask_cali.png");

	color &= 0xFFFFFF;
	color |= (uint32_t)(((double)opacity) * 2.55) << 24;

	vec4_from_rgba(&filter->color, color);
	calibr_filter_image_load(filter);
	filter->lock_aspect = false;
...
static void calibr_filter_defaults(obs_data_t *settings)
{
...
static obs_properties_t *calibr_filter_properties(void *data)
{
...
static void *calibr_filter_create(obs_data_t *settings, obs_source_t *context)
{
	struct calibr_filter_data *filter =
		bzalloc(sizeof(struct calibr_filter_data));
...
static void calibr_filter_destroy(void *data)
{
	struct calibr_filter_data *filter = data;
...
static void calibr_filter_tick(void *data, float seconds)
{
	struct calibr_filter_data *filter = data;
...
static void calibr_filter_render(void *data, gs_effect_t *effect)
{
	struct calibr_filter_data *filter = data;
...
struct obs_source_info calibr_filter = {
	.id = "calibr_filter",
	.type = OBS_SOURCE_TYPE_FILTER,
	.output_flags = OBS_SOURCE_VIDEO,
	.get_name = calibr_filter_get_name,
	.create = calibr_filter_create,
	.destroy = calibr_filter_destroy,
	.update = calibr_filter_update,
	.get_defaults = calibr_filter_defaults,
	.get_properties = calibr_filter_properties,
	.video_tick = calibr_filter_tick,
	.video_render = calibr_filter_render,
};
